...
namespace ShibpurConnectWebApp.Controllers.WebAPI
{
    ...
    public class EducationalHistoriesController : ApiController
    {
        ...
        public IQueryable<EducationalHistory> GetEducationalHistory()
        {
            return db.EducationalHistories;
        }

        // GET: api/EducationalHistories/userid
        [ResponseType(typeof(EducationalHistory))]
        public IHttpActionResult GetEducationalHistory(string userId)
        {
            var educationalHistory = db.EducationalHistories.Where(m => m.UserId == userId).ToList();
        ...
        [ResponseType(typeof(EducationalHistory))]
        public IHttpActionResult PostEducationalHistory(EducationalHistory educationalHistory)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            // add the guid id for this history
            educationalHistory.Id = Guid.NewGuid().ToString();

            // read the corresponding department id
            DepartmentsController DC = new DepartmentsController();
            IHttpActionResult actionResult = DC.GetDepartment(educationalHistory.Department);
            ...
            db.EducationalHistories.Add(educationalHistory);
            ...
            {
                if (EducationalHistoryExists(educationalHistory.Id))
                {
                    return Conflict();
                }
                else
        ...
        {
            EducationalHistory educationalHistory = db.EducationalHistories.Find(id);
            if (educationalHistory == null)
            {
                return NotFound();
            }

            db.EducationalHistories.Remove(educationalHistory);
            db.SaveChanges();
